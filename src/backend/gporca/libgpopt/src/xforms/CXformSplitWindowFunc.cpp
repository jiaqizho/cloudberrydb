...
void
CXformSplitWindowFunc::PopulateSelect(
	CMemoryPool *mp, CExpression *pexpr,
	COperator **ppSelectCopy)
{
	CLogicalSelect *pexprSelect =
								 ...
								 CExpression *pexpr) const
{
	...
	// extend logic from `Exfp()`
	{
		...
		if (pexprSequenceProject->Pop()->Eopid() != COperator::EopLogicalSequenceProject || 
		...
		if (CLogicalSequenceProject::PopConvert(pexprSequenceProject->Pop())->Pspt() ==
		...
		// No need check the `ScalarCmp` which in `LogicSelect` contains the 
		// key of `ProjectList` which in `SequenceProject`, Because the output
		// in the `SequenceProject` must contain the key.
	}
	...
	CExpression *pexprLocalSelect =  GPOS_NEW(mp) 
			CExpression(mp, pSelectCopy, pexprLocal, pexprScalarCmp);

	CExpression *pexprGlobal = GPOS_NEW(mp) CExpression(
		mp,
		GPOS_NEW(mp) CLogicalSequenceProject(
			mp, COperator::ESPType::EsptypeGlobalTwoStep, pds, pdrgpos, pdrgpwf),
			pexprLocalSelect, pexprProjectListGlobal);

	CExpression *pexprGlobalSelect =  GPOS_NEW(mp) 
		CExpression(mp, pexpr->Pop(), pexprGlobal, pexprScalarCmp);
